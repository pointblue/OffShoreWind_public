...
getATtable<-function(base="NeededDistData",tblnam,includeID=FALSE){
	...
	rcweights<-airtable(base = baseID, tables = tblnam)
...
getTablesToProcess<-function(atSourceTable,seasons){
	...
	for(tt in names(tablesList)){
		...
		#sourcesTable<-subset(sourcesTable,!is.na(datasetName)) #commenting out so that species without dataset paths entered can still be processed
...
batchImportRasters_toOSW<-function(inpJSON){
	...
	if(inherits(wgrd,"try-error")){
		if(flog==0){
			cat("Failed to copy base grid locally (save_object failed).",sep ="\n",file = zz, append=TRUE)
			if(logSessionInfo){
				cat(sitd,sep="\n",file=zz,append=TRUE)
			}
			cat("\n","\n",file = zz, append=TRUE)
	...
	if(!fegrd){
		if(flog==0){
			cat("Failed to copy base grid locally (file.exists is False).",sep ="\n",file = zz, append=TRUE)
			if(logSessionInfo){
				cat(sitd,sep="\n",file=zz,append=TRUE)
			}
			cat("\n","\n",file = zz, append=TRUE)
	...
	for(ii in 1:nrow(tablesToProcess)){
		...
		datasetNm<-as.character(basename(inpRast)) #  sourcing the datasetNm from the yaml, not AT
		layerNm<-as.character(paste0(gsub(" ", "_", sciName), "_", season)) # sourcing the layerNm from the yaml, not AT
		tmpRastNm=paste0(localTempDir,"/",layerNm, "_InputRaster.tif")
		tmpInpRast<-try(save_object(object=inpRast,bucket=inpBuck,file=tmpRastNm),silent=T) # save raster locally from S3 
		...
		if(!inherits(rastLayer,"try-error")){
			...
			if(!inherits(sppTable,"try-error")){
				...
				## Collect metadata
				dateCreated<-format(Sys.time(),"%Y-%m-%d %H:%M:%OS2")
