	...
	if(FALSE %in% w){
		...
		if(!jsonlite::validate(inpJSON)[[1]]){
			...
			if(!bktt){
				...
				inpYAML<-inplst$Records$s3$object$key  #This should be the full path to the yaml, right???
				if(nchar(inpYAML)<20 | !file.exists(inpYAML)){
					...
					if(inherits(ymlc,"try-error")){
						...
						seasons<-ifelse(ymlc$seasons==""|is.na(ymlc$seasons)|ymlc$seasons==NULL|toupper(ymlc$seasons)=="ALL",c("Summer","Fall","Winter","Spring"),ymlc$seasons)
